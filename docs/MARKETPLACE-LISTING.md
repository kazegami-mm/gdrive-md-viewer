...
Marketplace 推奨の **1280×800** に統一して `assets/screenshots/` に配置済み（5 枚）:

| ファイル | シーン |
|----------|--------|
| `assets/screenshots/01-sidebar.png` | サイドパネルの疑似整形プレビュー |
| `assets/screenshots/02-viewer.png` | 全画面ビューア（表示モード・整形描画） |
| `assets/screenshots/03-editor.png` | 全画面エディタ（編集モード・EasyMDE） |
| `assets/screenshots/04-split.png` | 全画面エディタ（分割モード・左ソース右プレビュー） |
| `assets/screenshots/05-fullwidth.png` | 全画面ビューア（全幅表示） |

> いずれも 1280×800（実機を手動撮影 → GDI+ で内接リサイズ）。最低 1 枚・推奨 3〜5 枚を満たす。
> 競合検出モーダルは自動化で再現できなかったため未撮影（手動で撮るなら `04-conflict.png` として追加）。
> 撮り直し・追加の手順は `docs/SCREENSHOT-GUIDE.md`。
